...
namespace GradeBook.GradeBooks
{
    public abstract class BaseGradeBook
    {
        ...
        public virtual void CalculateStudentStatistics(string name)
        {
            ...
            if (student == null)
            {
                System.Console.WriteLine($"{name} not present in {Name} GradeBook");
        ...
        public static dynamic ConvertToGradeBook(string json)
        {
            ...
            //var gradebookEnum = (from assembly in AppDomain.CurrentDomain.GetAssemblies()          //LINQ
            //                     from type in assembly.GetTypes()
            //                     where type.FullName == "GradeBook.Enums.GradeBookType"
            //                     select type).FirstOrDefault();

            /* AppDomain.CurrentDomain.GetAssemblies()    //alternative way
                 .SelectMany(asm => asm.GetTypes())
                 .FirstOrDefault(type => type.FullName == "GradeBook.Enums.GradeBookType");
            */
            ...
            Type gradeBook;
            GradeBookType x;
            Enum.TryParse(gradeBookType, out x);
            if (x == GradeBookType.Standard)
                gradeBook = typeof(StandardGradeBook);
            else
                gradeBook = typeof(RankedGradeBook);
            ...
            //    else
            //        gradeBookType = Enum.GetName(typeof(GradeBookType), int.Parse(gradeBookType));
            //}
