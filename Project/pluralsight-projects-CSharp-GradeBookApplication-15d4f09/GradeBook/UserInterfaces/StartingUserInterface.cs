...
namespace GradeBook.UserInterfaces
{
    public static class StartingUserInterface
    {
        ...
        public static void CreateCommand(string command)
        {
            ...
            if (type == "standard")
            {
                var standard_gradeBook = new StandardGradeBook(name,isWeighted);      //assign the name and weight to BaseGradebook class 
                Console.WriteLine($"Created {type} gradebook {name}.");
                GradeBookUserInterface.CommandLoop(standard_gradeBook);
            }
            else if (type == "ranked")
            {
                var ranked_gradeBook = new RankedGradeBook(name,isWeighted);
                Console.WriteLine($"Created {type} gradebook {name}.");
                GradeBookUserInterface.CommandLoop(ranked_gradeBook);
            }
            else
            {
